	...
	fileprivate func readEncryption() {
		...
		do {
			...
			for encryptedItem in encryptedData {
				...
				FREncrypter.shared.decrypt(atPath:		(bookBasePath as NSString).appendingPathComponent(encryptedFontPath),
				                           withMethod:	FREncryptionMethod(rawValue:encryptionMethod),
				                           andKey:		epubUID)
			}
		} catch {
